...
# Configure page
st.set_page_config(page_title="WA Bushfire Detector", page_icon="🔥", layout="wide")
st.title("🔥 WA Bushfire Detector")
st.markdown("**AI-powered bushfire detection + official WA alerts** (built with ❤️ in Perth)")
...
# Tabs
tab1, tab2, tab3 = st.tabs(["🖼️ AI Detection", "🚨 Current WA Incidents", "🗺️ Bushfire Prone Areas"])
...
with tab1:
  ...
  if uploaded_file is not None: # If the uploaded image is of type jpg, jpeg or png
    ...
    image = image.convert('RGB') # Convert to RGB and remove alpha if present
    ...
    with col2:
      st.image(annotated, caption="🔥 Detected fire/smoke areas", use_column_width=True)


    # Summary
    detections = results[0].boxes
    if len(detections) > 0: #ie if there are detections
      st.success(f"🚨 **ALERT**: {len(detections)} fire/smoke detection(s) found!")
      for box in detections:
        cls = int(box.cls[0])
        conf = float(box.conf[0])
        class_name = model.names[cls]
        st.write(f"• **{class_name}** detected with {conf:.1%} confidence")
...
st.markdown("---")
st.markdown("[Open full interactive map →](https://www.emergency.wa.gov.au)")
...
→ [Bush Fire Prone Areas 2024 (data.wa.gov.au)](https://catalogue.data.wa.gov.au/dataset/bush-fire-prone-areas-2024-obrm-021)
""")
...
with st.sidebar:
  ...
  st.markdown("---")
  st.caption("Made for Western Australia 🇦🇺 | February 2026")
  st.caption("Designed by Charles Owuama")
